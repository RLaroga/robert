...
public class FoodDelivery {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Hold_the_Elevator(4,2,6,4));
    ...
    public static int Hold_the_Elevator(int elevator_1, int elevator_2, int elevator_3, int arrival) {
        ...
        if (elevator_1 >= arrival) {
            elevators[ctr] = elevator_1;
            ctr++;
        }

        if (elevator_2 >= arrival) {
            elevators[ctr] = elevator_2;
            ctr++;
        }

        if (elevator_3 >= arrival) {
            elevators[ctr] = elevator_3;
            ctr++;
        }

        int firstElevatorToReachGroundFloor = elevators[0];

        for (int i = 0; i < elevators.length-1; i++) {
            int arrayCtr = i+1;
            if (firstElevatorToReachGroundFloor > elevators[arrayCtr] && elevators[arrayCtr] != 0) {
                firstElevatorToReachGroundFloor = elevators[arrayCtr];
            }
        }


        int result = firstElevatorToReachGroundFloor - arrival;
        System.out.println("firstElevatorToReachGroundFloor : " + firstElevatorToReachGroundFloor);
